export class BombaCombustivel {
  ...
  abastecerPorValor(valorAAbastecer: number) {
    ...
    l > this.quantidadeCombustivel
      ? console.log(`Combustível insuficiente: ${l.toFixed(1)}`)
      : console.log(
          `Foi abastecido com ${l.toFixed(
            1
          )}L, no valor de ${valorAAbastecer}. Resta na bomba ${this.quantidadeCombustivel.toFixed(
            1
          )}L`
        );
  }
  abastecerPorLitro(qtdLitro: number) {
    qtdLitro > this.quantidadeCombustivel
      ? console.log(`Combustível insuficiente: ${qtdLitro}`)
      : ((this.quantidadeCombustivel -= qtdLitro),
        console.log(
          `Foi abastecido com ${qtdLitro}L. Resta na bomba ${this.quantidadeCombustivel}L`
        ));
  }
